...
public class LoggerProxy {
    ...
    /// 日志输出函数
    public typealias LogFunc = (Level, () -> Date /* time */, StaticString /* file */, StaticString /* func */, Int /* line */, String /* label */, () -> String /* msg */ ) -> Void
    ...
    /// 默认控制台日志输出
    public static let defaultConsoleLogger: LogFunc = { level, now, file, funcName, line, tag, msg in
        let time = iso8601DateFormatter.string(from: now())
        let msg2 = msg().replacingOccurrences(of: "\n", with: "~n")
        print("\(time) [\(level.desc())] \(funcName):\(line) [\(tag)] \(msg2) ### \(file)")
    }

    /// 空白日志输出
    public static let defaultEmptyLogger: LogFunc = { _, _, _, _, _, _, _ in
    }
    ...
    public func appendLog(level: Level, tag: String, msg: @escaping AsyncLogMessage, file: StaticString, funcName: StaticString, line: Int) {
        if level.rawValue >= (logTagControl[tag] ?? acceptLevel).rawValue {
            let now: () -> Date = { Date() }
            let logFunc = logFunction
            if enableAsync {
                logQueue.async {
                    logFunc(level, now, file, funcName, line, tag, msg)
                }
            } else {
                logFunc(level, now, file, funcName, line, tag, msg)
            }
...
extension LoggerProxy {
    public static func VLog(tag: String, msg: @autoclosure @escaping AsyncLogMessage, file: StaticString = #file, funcName: StaticString = #function, line: Int = #line) {
        LoggerProxy.shared.appendLog(level: Level.Verbose, tag: tag, msg: msg, file: file, funcName: funcName, line: line)
    }

    public static func DLog(tag: String, msg: @autoclosure @escaping AsyncLogMessage, file: StaticString = #file, funcName: StaticString = #function, line: Int = #line) {
        LoggerProxy.shared.appendLog(level: Level.Debug, tag: tag, msg: msg, file: file, funcName: funcName, line: line)
    }

    public static func ILog(tag: String, msg: @autoclosure @escaping AsyncLogMessage, file: StaticString = #file, funcName: StaticString = #function, line: Int = #line) {
        LoggerProxy.shared.appendLog(level: Level.Info, tag: tag, msg: msg, file: file, funcName: funcName, line: line)
    }

    public static func WLog(tag: String, msg: @autoclosure @escaping AsyncLogMessage, file: StaticString = #file, funcName: StaticString = #function, line: Int = #line) {
        LoggerProxy.shared.appendLog(level: Level.Warn, tag: tag, msg: msg, file: file, funcName: funcName, line: line)
    }

    public static func ELog(tag: String, msg: @autoclosure @escaping AsyncLogMessage, file: StaticString = #file, funcName: StaticString = #function, line: Int = #line) {
